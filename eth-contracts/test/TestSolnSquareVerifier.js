...
//console.log(addedSolution.logs[0].event);
        ...
        let mintingSucceded = await this.contract.mintIt(account_two, 2, zokratesProof.proof, a, b, c, input, { from: account_one });
        let minted = mintingSucceded.receipt.status;
        assert.equal(minted, true, "Could not mint the token.");
        })
    })
})